-- Test3 v5: скрипт оптимизирован под наличие индексов на Account и AccountStatus.
-- Требует: IX_ACCOUNT_NB_BP4_COVER + IX_ACST_IDNACC_COVER (07_optional_indexes.sql).
-- Без этих индексов — использовать script_final.sql (v4).
--
-- Ключевое отличие: AA через явный CROSS JOIN (NrBank × BalAccount) → INNER JOIN Account.
-- Driving table ~168K строк, каждая делает index probe Account (NrBank, BalPrefix4) = MATCHCOLS=2.
