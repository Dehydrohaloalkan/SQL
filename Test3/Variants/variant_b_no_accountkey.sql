-- Вариант B: БЕЗ AccountKey (используется CASE-выражение).
-- С BalPrefix4 (материализованное поле) и SPBalAccount4 (раскрытая таблица).
-- Индексы: IX_ACCOUNT_NB_BP4_COVER, IX_ACST_IDNACC_COVER, IX_INFOYSR_DT_BP4.
-- Ожидание: MATCHCOLS=2 на Account (BalPrefix4 в индексе используется),
--           но index НЕ covering — нужен data page для NrAccount, NrEWallet, CdCurrency.
...
AA AS (
    SELECT
        ...
        A."BalPrefix4",
        CASE
            WHEN A."BalPrefix4" = '3119' AND A."NrEWallet" IS NOT NULL
...
SRA AS (
    SELECT
        CASE
            WHEN I."BalPrefix4" = '3119' AND I."NrEWallet" IS NOT NULL
