...
## Цель
Понять, почему скрипт выполняется ~50 минут, и какие изменения/индексы/переписывание запросов это исправят. Скрипт состоит только из селектов и CTE.
...
- **Параметры**: нет
- **Ограничения**: менять скрипт минимально; вероятно нужны надстройки над таблицами (индексы/статистики/материализация/таблицы-помощники) индексы менять нельзя, можно создавать новые
